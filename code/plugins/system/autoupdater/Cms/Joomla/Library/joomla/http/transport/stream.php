...
if (!class_exists('JHttpTransportStream') AND method_exists('JHttpTransport', 'isSupported'))
{
	...
	class JHttpTransportStream implements JHttpTransport
	{
		...
		public function request(
			...
			$contextOptions['http'] = isset($contextOptions['http']) ? array_merge($contextOptions['http'],
				$options) : $options;
			...
			$context = stream_context_create(
				array(
					...
					'ssl'  => array(
						...
						'cafile'       => $this->options->get('stream.certpath',
							dirname(__FILE__) . '/cacert.pem') ?: $this->options->get('certpath',
